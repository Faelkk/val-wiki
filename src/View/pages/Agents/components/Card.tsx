...
const Card = ({ agente }: CardProps) => {
  return (
    <Link to={`/agents/${agente.uuid}`} className="relative">
      <div className="h-64   overflow-hidden relative bg-agent before:content-[''] border-[1px] border-blueVariation2-500 before:block before:w-full before:h-full before:absolute before:top-0 before:border-[2rem] before:border-[#0F141A] before:box-border hover:scale-95 transition-all ">
        <ImageSkeleton
